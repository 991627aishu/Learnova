...
interface Lecture { id: string; title: string; type: string; order: number; videoUrl?: string; content?: string; }
      ...
      <CardContent className="p-5 space-y-5">
        <div className="flex items-center justify-between gap-4">
          <div className="flex-1 space-y-1.5">
            <label className="text-[10px] font-black uppercase tracking-widest text-primary/60 ml-1">Lecture Title</label>
            <Input 
              autoFocus
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <Button size="sm" onClick={onSubmit} disabled={!title.trim() || isSaving} className="font-bold px-6 shadow-md shadow-primary/10">
    ...
    <li ref={setNodeRef} style={style} className="flex items-center justify-between gap-4 text-sm text-foreground bg-background p-4 rounded-xl border border-border/40 shadow-sm group hover:border-primary/20 hover:shadow-md transition-all">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="min-w-0 flex-1">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">{lecture.type}</span>
        </div>
      </div>
      <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
        <Button variant="ghost" size="sm" className="h-9 px-3 rounded-lg font-bold text-muted-foreground hover:text-primary hover:bg-primary/5" onClick={() => onEdit(lecture.id, lecture.type)}>
  ...
  return (
    <div ref={setNodeRef} style={style} className="rounded-2xl border border-border/40 bg-card text-card-foreground shadow-sm mb-8 overflow-hidden">
      ...
      <div className="p-6 space-y-4">
        ...
        {!isAddingLecture && (
          <Button variant="outline" className="w-full mt-2 h-12 border-dashed border-primary/20 hover:border-primary hover:text-primary hover:bg-primary/5 transition-all font-bold rounded-xl" onClick={() => onAddLecture(section.id)}>
...
export function CurriculumBuilderPage() {
  ...
  const addSection = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["sections", courseId] }); setNewSectionTitle(""); setAddingSection(false); toast({ title: "Section added", variant: "success" }); },
  });
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      <div className="flex items-center justify-between">
        <div className="flex-1">
          ...
          <h1 className="text-4xl font-bold font-display tracking-tight text-foreground">Curriculum Builder</h1>
      ...
      <Card className="border-dashed bg-primary/5 border-primary/20">
        <CardContent className="pt-6">
          {addingSection ? (
            ...
            <Button variant="ghost" size="lg" className="w-full h-16 text-lg font-bold border-2 border-dashed border-primary/20 hover:bg-primary/5 text-primary/70 hover:text-primary rounded-2xl transition-all" onClick={() => setAddingSection(true)}>
